...
const QVector3D PLANE_COLOR(0.0, 0.0, 0.4);

const QVector3D SUBDIVISION_CUBE_POSITION(0, 0.01f, 0.0f);
const QVector3D SUBDIVISION_CUBE_COLOR(1,0,0);
...
std::unique_ptr<SubDivision::Mesh> cube_mesh;
std::unique_ptr<SubDivision::Mesh> cube_mesh_1;
std::unique_ptr<SubDivision::Mesh> cube_mesh_2;
std::unique_ptr<SubDivision::Mesh> cube_mesh_3;
std::unique_ptr<SubDivision::Mesh> cube_mesh_4;

std::unique_ptr<BasicGeometry> sub_cube;
std::unique_ptr<BasicGeometry> sub_cube_1;
std::unique_ptr<BasicGeometry> sub_cube_2;
std::unique_ptr<BasicGeometry> sub_cube_3;
std::unique_ptr<BasicGeometry> sub_cube_4;
std::unique_ptr<SubDivision::CCSubdivision> ccSubdivision;
...
void OGLAdapter::initializeGL() {
    ...
    // Subdivision Mesh
    ccSubdivision = std::make_unique<SubDivision::CCSubdivision>();

    cube_mesh = std::make_unique<SubDivision::Mesh>();
    cube_mesh_1 = std::make_unique<SubDivision::Mesh>();
    cube_mesh_2 = std::make_unique<SubDivision::Mesh>();
    cube_mesh_3 = std::make_unique<SubDivision::Mesh>();
    cube_mesh_4 = std::make_unique<SubDivision::Mesh>();

    cube_mesh->init(GeometryData::getSubdivisionCubeVertices(), GeometryData::getSubdivisionCubePolygon());

    ccSubdivision->run(*cube_mesh, *cube_mesh_1);
    ccSubdivision->run(*cube_mesh_1, *cube_mesh_2);
    ccSubdivision->run(*cube_mesh_2, *cube_mesh_3);
    ccSubdivision->run(*cube_mesh_3, *cube_mesh_4);

    sub_cube = std::make_unique<BasicGeometry>();
    sub_cube_1 = std::make_unique<BasicGeometry>();
    sub_cube_2 = std::make_unique<BasicGeometry>();

    sub_cube->init(cube_mesh->convertToTriangleMeshData(), Data_Type::ONLY_POSITION);
    sub_cube_1->init(cube_mesh_1->convertToTriangleMeshData(), Data_Type::ONLY_POSITION);
    sub_cube_2->init(cube_mesh_2->convertToTriangleMeshData(), Data_Type::ONLY_POSITION);
    ...
    ResourceManager::loadShader("sub_cube",
                                ":Resources/Shaders/basicGeometry.vert",
                                ":Resources/Shaders/basicGeometry.frag");
    ...
    ResourceManager::getShader("sub_cube").use().setVector3f("lightPos", LIGHT_POSITION);
    ResourceManager::getShader("sub_cube").use().setVector3f("viewPos", camera->position);
    ResourceManager::getShader("sub_cube").use().setVector3f("lightColor", LIGHT_COLOR);
    ResourceManager::getShader("sub_cube").use().setVector3f("objectColor", SUBDIVISION_CUBE_COLOR);
...
/*    model.setToIdentity();
    ...
    ResourceManager::getShader("sub_cube").use().setMatrix4f("model", model);*/
...
void OGLAdapter::paintGL() {
    ...
    model.setToIdentity();
    model.scale(0.02f);
    model.translate(SUBDIVISION_CUBE_POSITION);
    ResourceManager::getShader("sub_cube").use().setMatrix4f("model", model);
    sub_cube_2->draw();
...
void OGLAdapter::updateGL() {
    ...
    ResourceManager::getShader("sub_cube").use().setMatrix4f("projection", projection);
    ResourceManager::getShader("sub_cube").use().setMatrix4f("view", view);
    ResourceManager::getShader("sub_cube").use().setVector3f("viewPos", camera->position);
